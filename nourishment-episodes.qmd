...
I distinguish three three place clusters based on their first beach nourishment episode as seen in @fig-volume-cy-by-place:
...
p1 <- dta_for_plot |>
  ...
  labs(
    subtitle = "A: Cumulative volume by place",
    x = NULL,
...
p2 <- d_combined_with_est |>
  ...
  geom_vline(xintercept = c(1980, 2015), 
             lty = 2, linewidth = 0.15, alpha = 0.5) +
  geom_segment(aes(x = year_first, xend = year_last, y = place_label),
             alpha = 0.5, linewidth = 0.2) +
  geom_point(aes(x = year_first, y = place_label),
             alpha = 0.5, color = "dodgerblue", size = 2) +
  geom_point(aes(x = year_last, y = place_label),
             alpha = 0.5, color = "firebrick", size = 2) +
  labs(
    subtitle = "B: First and last year of nourishment",
    x = NULL,
    y = NULL
  )
...
#| fig-cap: "Length of shoreline nourished by volume"
#| fig-height: 6
#| fig-width: 12
#| warning: false
#| message: false
...
At some locations (example: Carolina Beach) there have been episodes of widely varying volume deposited on very similar lengths of beach while at other locations (example: Topsail beaches) it's the reverse. See @tbl-all-places-with-cy-distributions.
